...
function NotifRow({ notif, isDark }: NotifRowProps) {
...
type FilterValue = 'all' | 'sent' | 'failed' | 'pending';
    ...
    <DashboardLayout>
      ...
      <div className="mb-4 flex flex-wrap gap-2">
        <button
          ...
          className={`inline-flex items-center gap-2 rounded-xl border px-3 py-2 text-sm font-medium transition ${allFilterCls}`}
        >
        ...
        {(['sent', 'failed', 'pending'] as const).map((status) => {
          ...
          return (
            <button
              ...
              className={`inline-flex items-center gap-2 rounded-xl border px-3 py-2 text-sm font-medium capitalize transition ${btnCls}`}
            >
              {status}
            </button>
          );
      ...
      <div className={`overflow-hidden rounded-2xl border ${isDark ? 'border-zinc-800 bg-zinc-900' : 'border-zinc-100 bg-white'}`}>
        <div className="overflow-x-auto">
          <table className="w-full min-w-screen text-sm">
            <thead>
