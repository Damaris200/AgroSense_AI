...
describe('AppError subclasses (weather-service)', () => {
  it('NotFoundError has statusCode 404', () => {
    const err = new NotFoundError('Weather record not found');
    expect(err.statusCode).toBe(404);
    expect(err.message).toBe('Weather record not found');
    expect(err instanceof AppError).toBe(true);
  });

  it('ValidationError has statusCode 400', () => {
    const err = new ValidationError('farmId required');
    expect(err.statusCode).toBe(400);
    expect(err.message).toBe('farmId required');
  });
