...
// GET /api/recommendations — list AI recommendations for the authenticated user
router.get('/', requireAuth as any, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const userId = req.jwtUser!.sub;
    const url    = `${env.aiServiceUrl}/api/recommendations?userId=${encodeURIComponent(userId)}`;
